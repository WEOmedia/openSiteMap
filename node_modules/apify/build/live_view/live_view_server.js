...
class LiveViewServer {
    ...
    constructor(options = {}) {
        ...
        utils_1.checkParamOrThrow(screenshotDirectoryPath, 'options.screenshotDirectoryPath', 'String');
        utils_1.checkParamOrThrow(maxScreenshotFiles, 'options.maxScreenshotFiles', 'Number');
        utils_1.checkParamOrThrow(maxScreenshotFiles, 'options.snapshotTimeoutSecs', 'Number');
        utils_1.checkParamOrThrow(maxScreenshotFiles, 'options.maxSnapshotFrequencySecs', 'Number');
    ...
    async start() {
        ...
        try {
            ...
            utils_log_1.default.info('Live view web server started', { publicUrl: this.liveViewUrl });
        }
        catch (err) {
            utils_log_1.default.exception(err, 'Live view web server failed to start.');
            this._isRunning = false;
        }
    }
    /**
    ...
    async stop() {
        ...
        return new Promise((resolve) => {
            this.httpServer.close((err) => {
                this._isRunning = false;
                if (err)
                    utils_log_1.default.exception(err, 'Live view web server could not be stopped.');
                else
                    utils_log_1.default.info('Live view web server stopped.');
                resolve();
            });
        });
    }
    ...
    async serve(page) {
        ...
        try {
            ...
            const snapshot = await utils_2.addTimeoutToPromise(this._makeSnapshot(page), this.snapshotTimeoutMillis, 'LiveViewServer: Serving of Live View timed out.');
    ...
    async _makeSnapshot(page) {
        ...
        utils_log_1.default.info('Making live view snapshot.', { pageUrl });
    ...
    _pushSnapshot(snapshot) {
        ...
        utils_log_1.default.debug('Sending live view snapshot', { createdAt: snapshot.createdAt, pageUrl: snapshot.pageUrl });
    ...
    _deleteScreenshot(screenshotIndex) {
        unlink(this._getScreenshotPath(screenshotIndex))
            .catch(err => utils_log_1.default.exception(err, 'Cannot delete live view screenshot.'));
    }
    _setupHttpServer() {
    ...
    _socketConnectionHandler(socket) {
        this.clientCount++;
        utils_log_1.default.info('Live view client connected', { clientId: socket.id });
        socket.on('disconnect', (reason) => {
            this.clientCount--;
            utils_log_1.default.info('Live view client disconnected', { clientId: socket.id, reason });
        });
        socket.on('getLastSnapshot', () => {
            if (this.lastSnapshot) {
                utils_log_1.default.debug('Sending live view snapshot', { createdAt: this.lastSnapshot.createdAt, pageUrl: this.lastSnapshot.pageUrl });
