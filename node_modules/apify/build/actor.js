...
const waitForRunToFinish = async ({ actId, runId, token, waitSecs, taskId }) => {
    ...
    const shouldRepeat = () => {
        if (waitSecs && (Date.now() - startedAt) / 1000 >= waitSecs)
            return false;
        if (updatedRun && consts_1.ACT_JOB_TERMINAL_STATUSES.includes(updatedRun.status))
            return false;
        return true;
    };
    ...
    while (shouldRepeat()) {
        ...
        if (!updatedRun)
            await utils_2.sleep(250);
    }
    if (!updatedRun) {
        throw new errors_1.ApifyCallError({ id: runId, actId }, 'Apify.call() failed, cannot fetch actor run details from the server');
    }
    const { status } = updatedRun;
    if (status !== consts_1.ACT_JOB_STATUSES.SUCCEEDED
 ...
 */
exports.getApifyProxyUrl = (options = {}) => {
    // For backwards compatibility.
    // TODO: remove this when we release v1.0.0
    if (!options.groups && options.apifyProxyGroups) {
        utils_log_1.default.warning('Parameter `apifyProxyGroups` of Apify.getApifyProxyUrl() is deprecated!!! Use `groups` instead!');
        options.groups = options.apifyProxyGroups;
    }
    if (!options.session && options.apifyProxySession) {
        utils_log_1.default.warning('Parameter `apifyProxySession` of Apify.getApifyProxyUrl() is deprecated!!! Use `session` instead!');
        options.session = options.apifyProxySession;
    }
    const { groups, session, country, password = process.env[consts_1.ENV_VARS.PROXY_PASSWORD], hostname = process.env[consts_1.ENV_VARS.PROXY_HOSTNAME] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.PROXY_HOSTNAME], port = parseInt(process.env[consts_1.ENV_VARS.PROXY_PORT] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.PROXY_PORT], 10), 
    ...
    groupsParamName = 'opts.groups', sessionParamName = 'opts.session', countryParamName = 'opts.country', } = options;
    ...
    utils_1.checkParamOrThrow(groups, groupsParamName, 'Maybe [String]');
    utils_1.checkParamOrThrow(session, sessionParamName, 'Maybe Number | String');
    utils_1.checkParamOrThrow(country, countryParamName, 'Maybe String');
    utils_1.checkParamOrThrow(password, 'opts.password', 'String', getMissingParamErrorMgs('password', consts_1.ENV_VARS.PROXY_PASSWORD));
    utils_1.checkParamOrThrow(hostname, 'opts.hostname', 'String', getMissingParamErrorMgs('hostname', consts_1.ENV_VARS.PROXY_HOSTNAME));
    utils_1.checkParamOrThrow(port, 'opts.port', 'Number', getMissingParamErrorMgs('port', consts_1.ENV_VARS.PROXY_PORT));
    let username;
    if (groups || session || country) {
        const parts = [];
        if (groups && groups.length) {
            if (!groups.every(group => regexs_1.APIFY_PROXY_VALUE_REGEX.test(group)))
                throwInvalidProxyValueError('groups');
            parts.push(`groups-${groups.join('+')}`);
        }
        if (session) {
            if (!regexs_1.APIFY_PROXY_VALUE_REGEX.test(session))
                throwInvalidProxyValueError('session');
            parts.push(`session-${session}`);
        }
        if (country) {
            if (!constants_1.COUNTRY_CODE_REGEX.test(country))
                throwInvalidCountryCode(country);
