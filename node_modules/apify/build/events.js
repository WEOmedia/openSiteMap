...
exports.initializeEvents = () => {
    ...
    if (!eventsWsUrl) {
        utils_log_1.default.debug(`Apify.events: Environment variable ${consts_1.ENV_VARS.ACTOR_EVENTS_WS_URL} is not set, no events from Apify platform will be emitted.`);
        return;
    }
    ...
    eventsWs.on('message', (message) => {
        ...
        try {
            ...
            utils_log_1.default.exception(err, 'Apify.events: Cannot parse actor event');
        }
    });
    eventsWs.on('error', (err) => {
        ...
        if (err.message === 'WebSocket was closed before the connection was established')
            return;
        utils_log_1.default.exception(err, 'Apify.events: web socket connection failed');
    });
    eventsWs.on('close', () => {
        utils_log_1.default.warning('Apify.events: web socket has been closed');
        eventsWs = null;
    });
};
/**
