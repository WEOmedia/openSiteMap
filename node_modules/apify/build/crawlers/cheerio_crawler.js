...
class CheerioCrawler {
     ...
     */
    constructor(options = {}) {
        const { requestOptions, handlePageFunction, requestTimeoutSecs = 30, handlePageTimeoutSecs = 60, ignoreSslErrors = true, useApifyProxy = false, apifyProxyGroups, apifyProxySession, proxyUrls, additionalMimeTypes = [], 
        // Autoscaled pool shorthands
        minConcurrency, maxConcurrency, 
        // Basic crawler options
        requestList, requestQueue, maxRequestRetries, maxRequestsPerCrawl, handleFailedRequestFunction = this._defaultHandleFailedRequestFunction, autoscaledPoolOptions = DEFAULT_AUTOSCALED_POOL_OPTIONS, prepareRequestFunction, useSessionPool = false, sessionPoolOptions = {}, persistCookiesPerSession = false, } = options;
        ...
        utils_1.checkParamOrThrow(useApifyProxy, 'options.useApifyProxy', 'Maybe Boolean');
        utils_1.checkParamOrThrow(apifyProxyGroups, 'options.apifyProxyGroups', 'Maybe [String]');
        utils_1.checkParamOrThrow(apifyProxySession, 'options.apifyProxySession', 'Maybe String');
        ...
        if (useApifyProxy && proxyUrls)
            throw new Error('Cannot combine "options.useApifyProxy" with "options.proxyUrls"!');
    ...
    async _handleRequestFunction({ request, autoscaledPool, session }) {
        ...
        const { dom, isXml, body, contentType, response } = await utils_2.addTimeoutToPromise(this._requestFunction({ request, session }), this.requestTimeoutMillis, `CheerioCrawler: request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);
        ...
        const context = {
            ...
            get html() {
                utils_log_1.default.deprecated('CheerioCrawler: The "html" parameter of handlePageFunction is deprecated, use "body" instead.');
        ...
        return utils_2.addTimeoutToPromise(this.handlePageFunction(context), this.handlePageTimeoutMillis, `CheerioCrawler: handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
    }
    /**
    ...
    async _requestFunction({ request, session }) {
        ...
        const { type, charset } = utils_2.parseContentTypeFromResponse(responseStream);
    ...
    _getProxyUrl(session = {}) {
        if (this.useApifyProxy) {
            return actor_1.getApifyProxyUrl({
    ...
    async _defaultHandleFailedRequestFunction({ error, request }) {
        ...
        utils_log_1.default.exception(error, 'CheerioCrawler: Request failed and reached maximum retries', details);
    }
}
