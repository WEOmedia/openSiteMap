...
class PuppeteerCrawler {
    ...
    constructor(options) {
        ...
        requestList, requestQueue, maxRequestRetries, maxRequestsPerCrawl, handleFailedRequestFunction = this._defaultHandleFailedRequestFunction, autoscaledPoolOptions, 
        // PuppeteerPool options and shorthands
        puppeteerPoolOptions, launchPuppeteerFunction, launchPuppeteerOptions, sessionPoolOptions = {}, persistCookiesPerSession = false, useSessionPool = false, } = options;
        ...
        if (options.gotoTimeoutSecs && options.gotoFunction) {
            utils_log_1.default.warning('PuppeteerCrawler: You are using gotoTimeoutSecs with a custom gotoFunction. '
    ...
    async _handleRequestFunction({ request, autoscaledPool }) {
        ...
        try {
            ...
            await utils_2.addTimeoutToPromise(this.handlePageFunction({ page, request, autoscaledPool, puppeteerPool: this.puppeteerPool, response, session }), this.handlePageTimeoutMillis, `PuppeteerCrawler: handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
            if (session)
                session.markGood();
        }
        finally {
    ...
    async _defaultHandleFailedRequestFunction({ error, request }) {
        ...
        utils_log_1.default.exception(error, 'PuppeteerCrawler: Request failed and reached maximum retries', details);
    }
}
