...
const DEFAULT_OPTIONS = {
    ...
    handleFailedRequestFunction: ({ request }) => {
        ...
        utils_log_1.default.error('BasicCrawler: Request failed and reached maximum retries', details);
    },
...
class BasicCrawler {
     ...
     */
    constructor(options) {
        const { requestList, requestQueue, handleRequestFunction, handleRequestTimeoutSecs, handleFailedRequestFunction, maxRequestRetries, maxRequestsPerCrawl, autoscaledPoolOptions, sessionPoolOptions, useSessionPool, 
        // AutoscaledPool shorthands
        minConcurrency, maxConcurrency, } = _.defaults({}, options, DEFAULT_OPTIONS);
        ...
        this.stats = new statistics_1.default({ logMessage: 'Crawler request statistics:' });
        ...
        const basicCrawlerAutoscaledPoolConfiguration = {
            ...
            isTaskReadyFunction: async () => {
                if (isMaxPagesExceeded()) {
                    if (shouldLogMaxPagesExceeded) {
                        utils_log_1.default.info('BasicCrawler: Crawler reached the maxRequestsPerCrawl limit of '
            ...
            isFinishedFunction: async () => {
                if (isMaxPagesExceeded()) {
                    utils_log_1.default.info(`BasicCrawler: Earlier, the crawler reached the maxRequestsPerCrawl limit of ${maxRequestsPerCrawl} requests `
                ...
                if (isFinished) {
                    ...
                    utils_log_1.default.info(reason);
                }
                return isFinished;
            },
        };
    ...
    async run() {
        ...
        try {
            ...
            utils_log_1.default.info('Crawler final request statistics:', finalStats);
        }
    }
    async _pauseOnMigration() {
        await this.autoscaledPool.pause(SAFE_MIGRATION_WAIT_MILLIS)
            ...
            if (err.message.includes('running tasks did not finish')) {
                utils_log_1.default.error('BasicCrawler: The crawler was paused due to migration to another host, '
        ...
        if (this.requestList) {
            ...
            await this.requestList.persistState()
                ...
                if (err.message.includes('Cannot persist state.')) {
                    utils_log_1.default.error('BasicCrawler: The crawler attempted to persist its request list\'s state and failed due to missing or '
                    ...
                    utils_log_1.default.exception(err, 'BasicCrawler: An unexpected error occured when the crawler '
    ...
    async _fetchNextRequest() {
        ...
        try {
            ...
            utils_log_1.default.exception(err, 'RequestQueue.addRequest() failed, reclaiming request back to the list', { request });
    ...
    async _runTaskFunction() {
        ...
        try {
            await utils_2.addTimeoutToPromise(this.handleRequestFunction({ request, autoscaledPool: this.autoscaledPool, session }), this.handleRequestTimeoutMillis, `BasicCrawler: handleRequestFunction timed out after ${this.handleRequestTimeoutMillis / 1000} seconds.`);
            ...
            try {
                ...
                utils_log_1.default.exception(secondaryError, 'BasicCrawler: runTaskFunction error handler threw an exception. '
    ...
    async _requestFunctionErrorHandler(error, request, source) {
        ...
        if (!request.noRetry && request.retryCount < this.maxRequestRetries) {
            request.retryCount++;
            utils_log_1.default.exception(error, 'BasicCrawler: handleRequestFunction failed, reclaiming failed request back to the list or queue', _.pick(request, 'url', 'retryCount', 'id'));
