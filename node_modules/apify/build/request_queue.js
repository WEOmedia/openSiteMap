...
class RequestQueue {
    ...
    async fetchNextRequest() {
        ...
        if (this.inProgress.has(nextRequestId) || this.recentlyHandled.get(nextRequestId)) {
            utils_log_1.default.warning('Queue head returned a request that is already in progress?!', {
                nextRequestId,
        ...
        if (!request) {
            utils_log_1.default.debug('Cannot find a request from the beginning of queue, will be retried later', { nextRequestId });
            setTimeout(() => {
                this.inProgress.delete(nextRequestId);
        ...
        if (request.handledAt) {
            utils_log_1.default.debug('Request fetched from the beginning of queue was already handled', { nextRequestId });
    ...
    async reclaimRequest(request, options = {}) {
        ...
        setTimeout(() => {
            if (!this.inProgress.has(request.id)) {
                utils_log_1.default.warning('The request is no longer marked as in progress in the queue?!', { requestId: request.id });
                return;
            }
            this.inProgress.delete(request.id);
            // Performance optimization: add request straight to head if possible
    ...
    async _ensureHeadIsNonEmpty(ensureConsistency = false, limit = Math.max(this.inProgressCount() * exports.QUERY_HEAD_BUFFER, exports.QUERY_HEAD_MIN_LENGTH), iteration = 0) {
        ...
        if (prevLimit >= consts_1.REQUEST_QUEUE_HEAD_MAX_LIMIT) {
            utils_log_1.default.warning(`RequestQueue: Reached the maximum number of requests in progress: ${consts_1.REQUEST_QUEUE_HEAD_MAX_LIMIT}.`);
        }
        ...
        if (shouldRepeatForConsistency) {
            ...
            utils_log_1.default.info(`RequestQueue: Waiting for ${delayMillis}ms before considering the queue as finished to ensure that the data is consistent.`);
    ...
    /** @ignore */
    async delete() {
        utils_log_1.default.deprecated('requestQueue.delete() is deprecated. Please use requestQueue.drop() instead. '
