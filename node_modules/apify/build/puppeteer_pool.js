...
const DEFAULT_OPTIONS = {
    ...
    launchPuppeteerFunction: launchPuppeteerOptions => puppeteer_1.launchPuppeteer(launchPuppeteerOptions),
...
const deleteDiskCacheDir = (diskCacheDir) => {
    utils_log_1.default.debug('PuppeteerPool: Deleting disk cache directory', { diskCacheDir });
    return rimrafAsync(diskCacheDir)
        .catch((err) => {
        utils_log_1.default.warning('PuppeteerPool: Cannot delete Chrome disk cache directory', { diskCacheDir, errorMessage: err.message });
    });
};
/**
...
class PuppeteerPool {
     ...
     */
    constructor(options = {}) {
        const { maxOpenPagesPerInstance, retireInstanceAfterRequestCount, launchPuppeteerFunction, puppeteerOperationTimeoutSecs, instanceKillerIntervalMillis, instanceKillerIntervalSecs, killInstanceAfterMillis, killInstanceAfterSecs, launchPuppeteerOptions, recycleDiskCache, useIncognitoPages, proxyUrls, useLiveView, sessionPool, } = _.defaults({}, options, DEFAULT_OPTIONS);
        ...
        if (instanceKillerIntervalMillis) {
            utils_log_1.default.deprecated('PuppeteerPool: options.instanceKillerIntervalMillis is deprecated, use options.instanceKillerIntervalSecs instead.');
        }
        ...
        if (killInstanceAfterMillis) {
            utils_log_1.default.deprecated('PuppeteerPool: options.killInstanceAfterMillis is deprecated, use options.killInstanceAfterSecs instead.');
        }
    ...
    _launchInstance() {
        ...
        utils_log_1.default.debug('PuppeteerPool: Launching new browser', { id });
    ...
    async _initBrowser(browserPromise, instance) {
        ...
        try {
            ...
            utils_log_1.default.exception(err, 'PuppeteerPool: Browser launch failed', { id });
        ...
        browser.on('disconnected', () => {
            ...
            if (!instance.killed)
                utils_log_1.default.error('PuppeteerPool: Puppeteer sent "disconnect" event. Maybe it crashed???', { id });
    ...
    _retireInstance(instance) {
        ...
        if (!this.activeInstances[id])
            return utils_log_1.default.debug('PuppeteerPool: browser is retired already', { id });
        utils_log_1.default.debug('PuppeteerPool: Retiring browser', { id });
    ...
    _killRetiredInstances() {
        utils_log_1.default.debug('PuppeteerPool: Retired browsers count', { count: _.values(this.retiredInstances).length });
        _.mapObject(this.retiredInstances, async (instance) => {
            ...
            if (Date.now() - instance.lastPageOpenedAt > this.killInstanceAfterMillis) {
                utils_log_1.default.debug('PuppeteerPool: killing retired browser after period of inactivity', { id: instance.id, killInstanceAfterMillis: this.killInstanceAfterMillis }); // eslint-disable-line max-len
            ...
            try {
                ...
                if (pages.length <= 1) {
                    utils_log_1.default.debug('PuppeteerPool: Killing retired browser because it has no open tabs', { id: instance.id });
                    this._killInstance(instance);
                }
            }
            catch (err) {
                utils_log_1.default.exception(err, 'PuppeteerPool: Browser.pages() failed', { id: instance.id });
                this._killInstance(instance);
            }
        });
    }
    ...
    async _killInstance(instance) {
        ...
        if (killed)
            return;
        utils_log_1.default.debug('PuppeteerPool: Killing browser', { id });
        delete this.retiredInstances[id];
        const recycleDiskCache = () => {
            if (!instance.recycleDiskCacheDir)
                return;
            utils_log_1.default.debug('PuppeteerPool: Recycling disk cache dir', { id, diskCacheDir: instance.recycleDiskCacheDir });
        ...
        try {
            ...
            utils_log_1.default.exception(err, 'PuppeteerPool: Cannot close the browser instance, it will be killed forcibly.', { id });
        }
    }
    /**
    ...
    async _openNewTab() {
        ...
        try {
            ...
            const page = await utils_2.addTimeoutToPromise(context.newPage(), this.puppeteerOperationTimeoutMillis, 'PuppeteerPool: browser.newPage() timed out.');
            await this._focusOldestTab(browser).catch(() => utils_log_1.default.debug('Could not focus oldest tab.'));
    ...
    _decoratePage(page) {
        ...
        page.close = async (...args) => {
            ...
            await originalPageClose.apply(page, args)
                .catch((err) => {
                utils_log_1.default.debug('PuppeteerPool: Page.close() failed', { errorMessage: err.message, id: instance.id });
            });
            const context = page.browserContext();
            if (context.isIncognito()) {
                await context.close().catch((err) => {
                    utils_log_1.default.debug('PuppeteerPool: Context.close() failed', { errorMessage: err.message, id: instance.id });
                });
            }
        };
        page.once('error', (error) => {
            utils_log_1.default.exception(error, 'PuppeteerPool: Page crashed.');
            page.close();
        });
        page.once('close', () => {
            instance.activePages--;
    ...
    async destroy() {
        ...
        const closePromises = browserPromises.map(async (browserPromise) => {
            ...
            if (browser.recycleDiskCacheDir)
                await deleteDiskCacheDir(browser.recycleDiskCacheDir);
        });
        try {
            ...
            utils_log_1.default.exception(err, 'PuppeteerPool: Cannot close the browsers.');
        }
        if (this.liveViewServer) {
            await this.liveViewServer.stop().catch(err => utils_log_1.default.exception(err, 'PuppeteerPool: Cannot close LiveViewServer.'));
        }
    }
    /**
    ...
    async retire(browser) {
        ...
        utils_log_1.default.debug('PuppeteerPool: Browser is retired already');
    }
    /**
    ...
    async recyclePage(page) {
        ...
        if (this.reusePages) {
            ...
            try {
                await utils_2.addTimeoutToPromise(page.close(), this.puppeteerOperationTimeoutMillis, 'PuppeteerPool: page.close() timed out.');
            }
            catch (err) {
                utils_log_1.default.debug('PuppeteerPool: page.close() failed.', { reason: err && err.message });
            }
        }
    }
    /**
    ...
    async serveLiveViewSnapshot(page) {
        ...
        const snapshotPromise = this.liveViewServer.serve(page)
            .catch(err => utils_log_1.default.debug('Live View failed to be served.', { message: err.message }));
    ...
    async _retireBrowserWithSession(session) {
        ...
        if (isInstanceRunning) {
            ...
            try {
                ...
                utils_log_1.default.debug('Could not retire instances ', e);
            }
        }
    }
    _killInstanceWithNoPages(instance) {
        ...
        if (instance.activePages === 0 && this.retiredInstances[id]) {
            ...
            setTimeout(() => {
                utils_log_1.default.debug('PuppeteerPool: Killing retired browser because it has no active pages', { id });
                this._killInstance(instance);
