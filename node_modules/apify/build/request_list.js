...
class RequestList {
    ...
    constructor(options = {}) {
        ...
        if (persistSourcesKey) {
            utils_log_1.default.deprecated('RequestList: options.persistSourcesKey is deprecated. Use options.persistRequestsKey.');
        }
    ...
    async persistState() {
        ...
        try {
            ...
            utils_log_1.default.exception(err, 'RequestList attempted to persist state, but failed.');
        }
    }
    /**
    ...
    _restoreState(state) {
        ...
        if (deleteFromInProgress.length) {
            utils_log_1.default.warning('RequestList\'s in-progress field is not consistent, skipping invalid in-progress entries', { deleteFromInProgress });
    ...
    async _loadStateAndPersistedRequests() {
        ...
        if (this.initialState) {
            ...
            utils_log_1.default.debug('RequestList: Loaded state from options.state argument.');
        }
        else if (this.persistStateKey) {
            state = key_value_store_1.getValue(this.persistStateKey);
            if (state)
                utils_log_1.default.debug('RequestList: Loaded state from key value store using the persistStateKey.');
        }
        if (this.persistRequestsKey) {
            persistedRequests = await key_value_store_1.getValue(this.persistRequestsKey);
            if (persistedRequests)
                utils_log_1.default.debug('RequestList: Loaded requests from key value store using the persistRequestsKey.');
        }
    ...
    async _addFetchedRequests(source, fetchedRequests) {
        ...
        utils_log_1.default.info('RequestList: Fetched and loaded Requests from a remote resource.', {
            requestsFromUrl,
            regex,
            fetchedCount,
            importedCount,
    ...
    async _fetchRequestsFromUrl(source) {
        ...
        if (!urlsArr.length) {
            utils_log_1.default.warning('RequestList: list fetched, but it is empty.', { requestsFromUrl, regex });
            return [];
        }
    ...
    _addRequest(source) {
        ...
        if (!this.uniqueKeyToIndex.hasOwnProperty(uniqueKey)) { // eslint-disable-line no-prototype-builtins
            ...
            utils_log_1.default.warning(`RequestList: Duplicate uniqueKey: ${uniqueKey} found while the keepDuplicateUrls option was set. Check your sources' unique keys.`); // eslint-disable-line max-len
        }
    }
    /**
