...
class AutoscaledPool {
     ...
     */
    constructor(options = {}) {
        const { maxConcurrency, minConcurrency, desiredConcurrency, desiredConcurrencyRatio, scaleUpStepRatio, scaleDownStepRatio, maybeRunIntervalSecs, loggingIntervalSecs, autoscaleIntervalSecs, runTaskFunction, isFinishedFunction, isTaskReadyFunction, systemStatusOptions, snapshotterOptions, } = _.defaults({}, options, DEFAULT_OPTIONS);
        utils_1.checkParamOrThrow(maxConcurrency, 'options.maxConcurrency', 'Number');
        utils_1.checkParamOrThrow(minConcurrency, 'options.minConcurrency', 'Number');
        utils_1.checkParamOrThrow(desiredConcurrency, 'options.desiredConcurrency', 'Maybe Number');
        utils_1.checkParamOrThrow(desiredConcurrencyRatio, 'options.desiredConcurrencyRatio', 'Number');
        utils_1.checkParamOrThrow(scaleUpStepRatio, 'options.scaleUpStepRatio', 'Number');
        utils_1.checkParamOrThrow(scaleDownStepRatio, 'options.scaleDownStepRatio', 'Number');
        utils_1.checkParamOrThrow(maybeRunIntervalSecs, 'options.maybeRunIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(loggingIntervalSecs, 'options.loggingIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(autoscaleIntervalSecs, 'options.autoscaleIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(runTaskFunction, 'options.runTaskFunction', 'Function');
        utils_1.checkParamOrThrow(isFinishedFunction, 'options.isFinishedFunction', 'Function');
        utils_1.checkParamOrThrow(isTaskReadyFunction, 'options.isTaskReadyFunction', 'Function');
        utils_1.checkParamOrThrow(systemStatusOptions, 'options.systemStatusOptions', 'Maybe Object');
        utils_1.checkParamOrThrow(snapshotterOptions, 'options.snapshotterOptions', 'Maybe Object');
        ...
        if (!ssoCopy.snapshotter)
            ssoCopy.snapshotter = new snapshotter_1.default(snapshotterOptions);
    ...
    setMaxConcurrency(maxConcurrency) {
        utils_log_1.default.deprecated('AutoscaledPool.setMaxConcurrency() is deprecated, use the "maxConcurrency" property instead');
        this._maxConcurrency = maxConcurrency;
    }
    /**
     * @ignore
    ...
    set minConcurrency(value) {
        utils_1.checkParamOrThrow(value, 'value', 'Number');
    ...
    async _maybeRunTask(intervalCallback) {
        ...
        if (!isSystemIdle && this._currentConcurrency >= this._minConcurrency) {
            utils_log_1.default.perf('AutoscaledPool: Task will not be run. System is overloaded.', currentStatus);
            return done();
        }
        // - a task is ready.
        ...
        try {
            ...
            if (this.reject) {
                ...
                utils_log_1.default.exception(err, 'AutoscaledPool: isTaskReadyFunction failed');
                this.reject(err);
            }
        }
        finally {
        ...
        try {
            ...
            if (this.reject) {
                // No need to log all concurrent errors.
                utils_log_1.default.exception(err, 'AutoscaledPool: runTaskFunction failed.');
                this.reject(err);
            }
        }
    }
    ...
    _autoscale(intervalCallback) {
        ...
        if (this.loggingIntervalMillis > 0) {
            ...
            if (now > this.lastLoggingTime + this.loggingIntervalMillis) {
                ...
                utils_log_1.default.info('AutoscaledPool state', {
    ...
    _scaleUp(systemStatus) {
        ...
        utils_log_1.default.debug('AutoscaledPool: scaling up', {
    ...
    _scaleDown(systemStatus) {
        ...
        utils_log_1.default.debug('AutoscaledPool: scaling down', {
            oldConcurrency: this._desiredConcurrency + step,
            newConcurrency: this._desiredConcurrency,
            systemStatus,
        });
    ...
    async _maybeFinish() {
        ...
        try {
            ...
            if (this.reject) {
                // No need to log all concurrent errors.
                utils_log_1.default.exception(err, 'AutoscaledPool: isFinishedFunction failed.');
                this.reject(err);
            }
        }
        finally {
