...
class Snapshotter {
     ...
     */
    constructor(options = {}) {
        const { eventLoopSnapshotIntervalSecs, cpuSnapshotIntervalSecs, memorySnapshotIntervalSecs, clientSnapshotIntervalSecs, snapshotHistorySecs, maxBlockedMillis, maxUsedMemoryRatio, maxUsedCpuRatio, maxClientErrors, } = _.defaults({}, options, DEFAULT_OPTIONS);
        utils_1.checkParamOrThrow(eventLoopSnapshotIntervalSecs, 'options.eventLoopSnapshotIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(memorySnapshotIntervalSecs, 'options.memorySnapshotIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(cpuSnapshotIntervalSecs, 'options.cpuSnapshotIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(snapshotHistorySecs, 'options.snapshotHistorySecs', 'Number');
        utils_1.checkParamOrThrow(clientSnapshotIntervalSecs, 'options.clientSnapshotIntervalSecs', 'Number');
        utils_1.checkParamOrThrow(maxBlockedMillis, 'options.maxBlockedMillis', 'Number');
        utils_1.checkParamOrThrow(maxUsedMemoryRatio, 'options.maxUsedMemoryRatio', 'Number');
        utils_1.checkParamOrThrow(maxUsedCpuRatio, 'options.maxUsedCpuRatio', 'Number');
        utils_1.checkParamOrThrow(maxClientErrors, 'options.maxClientErrors', 'Number');
    ...
    async _snapshotMemoryOnLocal(intervalCallback) {
        try {
            const now = new Date();
            const memInfo = await utils_2.getMemoryInfo();
            ...
            utils_log_1.default.exception(err, 'Snapshotter: Memory snapshot failed.');
        }
        finally {
            intervalCallback();
        }
    ...
    _memoryOverloadWarning({ memCurrentBytes }) {
        ...
        if (isCriticalOverload) {
            ...
            utils_log_1.default.warning('Memory is critically overloaded. '
    ...
    _snapshotClient(intervalCallback) {
        ...
        const allErrorCounts = utils_2.apifyClient.stats.rateLimitErrors;
    ...
    async _ensureCorrectMaxMemory() {
        if (this.maxMemoryBytes)
            return;
        const { totalBytes } = await utils_2.getMemoryInfo();
        if (utils_2.isAtHome()) {
            ...
            utils_log_1.default.info(`AutoscaledPool: Setting max memory of this run to ${Math.round(this.maxMemoryBytes / 1024 / 1024)} MB. Use the ${consts_1.ENV_VARS.MEMORY_MBYTES} environment variable to override it.`); // eslint-disable-line max-len
        }
    }
}
