...
exports.validateInputSchema = exports.parseAjvError = undefined;

var _underscore = require('underscore');

var _underscore2 = _interopRequireDefault(_underscore);

var _input_schema = require('./input_schema.json');

var _input_schema2 = _interopRequireDefault(_input_schema);

var _intl = require('./intl');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const { definitions } = _input_schema2.default;
 ...
 */
const parseAjvError = exports.parseAjvError = (error, rootName, properties = {}, input = {}) => {
    ...
    if (error.keyword === 'type') {
        ...
        message = (0, _intl.m)('inputSchema.validation.generic', { rootName, fieldKey, message: error.message });
    } else if (error.keyword === 'required') {
        fieldKey = error.params.missingProperty;
        message = (0, _intl.m)('inputSchema.validation.required', { rootName, fieldKey });
    } else if (error.keyword === 'additionalProperties') {
        fieldKey = error.params.additionalProperty;
        message = (0, _intl.m)('inputSchema.validation.additionalProperty', { rootName, fieldKey });
    } else {
        fieldKey = error.dataPath.split('.').pop();
        message = (0, _intl.m)('inputSchema.validation.generic', { rootName, fieldKey, message: error.message });
    }
...
const validateBasicStructure = (validator, obj) => {
    const schemaWithoutProperties = Object.assign({}, _input_schema2.default);
    schemaWithoutProperties.properties = Object.assign({}, _input_schema2.default.properties, {
...
const validateField = (validator, fieldSchema, fieldKey) => {
    const matchingDefinitions = Object.values(definitions).filter(definition => {
        ...
        : _underscore2.default.isArray(fieldSchema.type);
    });

    // There is not matching definition.
    if (matchingDefinitions.length === 0) {
        const errorMessage = (0, _intl.m)('inputSchema.validation.noMatchingDefinition', { fieldKey });
        throw new Error(`Input schema is not valid (${errorMessage})`);
    }
 ...
 */
const validateInputSchema = exports.validateInputSchema = (validator, inputSchema) => {
    ...
    _underscore2.default.mapObject(inputSchema.properties, (fieldSchema, fieldKey) => validateField(validator, fieldSchema, fieldKey));

    // Finally just to be sure run validation against the whole shema.
    validateAgainstSchemaOrThrow(validator, inputSchema, _input_schema2.default, 'schema');
};